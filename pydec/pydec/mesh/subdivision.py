...
from scipy import concatenate, ravel, array, ndim, vstack, hstack, zeros, arange, tile
...
def triangulate_ncube(vertices, indices):
    ...
    for i in range(n_dims):
        ...
        if (n_dims - 1 - i) % 2 == n_dims % 2:
            ...
            faces[(2 * i + 1) * n_cubes:(2 * i + 2) * n_cubes, 0] = faces[(2 * i + 1) * n_cubes:(2 * i + 2) * n_cubes,
                                                                    1]
            faces[(2 * i + 1) * n_cubes:(2 * i + 2) * n_cubes, 1] = temp
        else:
            # flip 0
            temp = faces[(2 * i + 0) * n_cubes:(2 * i + 1) * n_cubes, 0].copy()
            faces[(2 * i + 0) * n_cubes:(2 * i + 1) * n_cubes, 0] = faces[(2 * i + 0) * n_cubes:(2 * i + 1) * n_cubes,
                                                                    1]
