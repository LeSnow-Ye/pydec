...
def read_basic(fid, header):
    try:
        dimensions = split_on_comma(header['dims'])
    except:
...
def read_ndarray(fid, header):
    try:
        ...
        raise FileFormatError('Unable to determine rank')

    try:
        dims = split_on_comma(header['dims'])
    except:
        raise FileFormatError('Unable to determine dims')
    ...
    try:
        ...
        raise FileFormatError('Unable to determine format')
...
def read_sparse(fid, header):
    ...
    try:
        format = header['sptype']
    except:
        raise FileFormatError('Unable to determine sparse format')
