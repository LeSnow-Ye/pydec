from numpy import hstack, arange, alltrue, array, \
    lexsort, hsplit, empty, ascontiguousarray, ndim
...
def cube_array_search(k_face_array, k_faces):
    ...
    lookup_grid[:] = -1
    lookup_grid[hsplit(k_face_array, k_face_array.shape[1])] = arange(k_face_array.shape[0],
                                                                      dtype=k_faces.dtype).reshape((-1, 1))
